...
本仓库中的 Otto 机器人在原有动作能力之外，重点增加了两类交互改进：

- **带嘴巴的 GIF 表情**：原开源 Otto 表情没有独立嘴巴。本版本将嘴巴直接绘入 GIF 表情帧，并在说话时切换到对应的 `_talking` 表情，避免运行时 canvas 绘制带来的屏幕卡顿。
- **对话式番茄钟**：用户可以通过语音设置、查询和取消番茄钟；双击 BOOT 按键时，屏幕显示剩余倒计时 10 秒，然后自动恢复正常表情。

otto 机器人仍保留丰富的动作能力，包括行走、转向、跳跃、摇摆等多种舞蹈动作。

### 新增番茄钟工具

| MCP工具名称 | 描述 | 参数说明 |
|-----------|------|---------|
| self.pomodoro.start | 开始本地番茄钟 | **duration_minutes**: 分钟数，1-1440，默认25<br>**label**: 计时名称，默认 Pomodoro |
| self.pomodoro.cancel | 取消当前番茄钟 | 无参数 |
| self.pomodoro.get_status | 查询番茄钟状态 | 返回是否运行、剩余秒数、总秒数和名称 |
| self.pomodoro.show | 临时显示倒计时 | 无参数，屏幕显示 10 秒 |
...
```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.pomodoro.start","arguments":{"duration_minutes":25,"label":"study"}},"id":20}
```

#### 显示番茄钟倒计时

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.pomodoro.show","arguments":{}},"id":21}
```

#### 取消番茄钟

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.pomodoro.cancel","arguments":{}},"id":22}
```
...
```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.get_device_status","arguments":{}},"id":23}
```

#### 设置音量（0~100）

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.audio_speaker.set_volume","arguments":{"volume":70}},"id":24}
```

#### 重启设备

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.reboot","arguments":{}},"id":25}
```
...
```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.otto.servo_sequences","arguments":{"sequence":"{\"a\":[{\"s\":{\"ll\":110,\"rl\":70},\"v\":800},{\"s\":{\"ll\":90,\"rl\":90},\"v\":800}],\"d\":0}"}},"id":26}
```

#### 振荡器模式（双臂摆动）

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.otto.servo_sequences","arguments":{"sequence":"{\"a\":[{\"osc\":{\"a\":{\"lh\":30,\"rh\":30},\"o\":{\"lh\":90,\"rh\":90},\"ph\":{\"rh\":180},\"p\":500,\"c\":5.0}}]}"}},"id":27}
```

#### 振荡器模式（左右摇摆波浪）

```json
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"self.otto.servo_sequences","arguments":{"sequence":"{\"a\":[{\"osc\":{\"a\":{\"ll\":20,\"rl\":20},\"o\":{\"ll\":90,\"rl\":90},\"ph\":{\"rl\":180},\"p\":600,\"c\":5.0}}]}"}},"id":28}
```
